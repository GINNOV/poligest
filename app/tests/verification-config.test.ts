...
describe("verification configuration", () => {
  it("exposes one local command that runs production build, tests, browser smoke, and lint", () => {
    const packageJson = JSON.parse(readFileSync(resolve(appRoot, "package.json"), "utf8")) as {
      scripts?: Record<string, string>;
    };

    expect(packageJson.scripts?.verify).toBe("npm run build && npm test && npm run test:smoke && npm run lint");
  });

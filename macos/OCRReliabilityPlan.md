...
- The rebuilt app was relaunched from `macos/build/DerivedData/Build/Products/Debug/ScanID.app`, and macOS menu automation reported the expected live-camera idle state: `New Scan` enabled, `Freeze Current Camera Frame` enabled, and `Export OCR Fixture...` disabled until a frame is frozen.
